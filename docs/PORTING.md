...
_Note_: Although Vulkan probably works on other platforms, it has been tested only on Linux.
...
| **`src/ship/`** | Platform-agnostic code: resource/archive management, audio backends (SDL, Null), controller abstractions, configuration, scripting/DLL loading, security/keystore, and utility functions. |
| **`src/fast/`** | The Fast3D Renderer. Interprets N64 display lists as commands for modern GPUs. Supports OpenGL, Direct3D 11, Metal, and Vulkan backends. |
