...
if (NOT CMAKE_SYSTEM_NAME STREQUAL "iOS" AND NOT CMAKE_SYSTEM_NAME STREQUAL "Android")
    ...
    # gfx_vulkan.cpp compiles GLSL at runtime through shaderc and includes its header
    # unconditionally, so a loader without shaderc cannot build. Probe for it first and
    # treat a miss the same as no Vulkan at all.
    set(_lus_shaderc_target "")

    if(Vulkan_FOUND AND WIN32 AND DEFINED ENV{VULKAN_SDK})
        ...
        if(Vulkan_shaderc_shared_LIBRARY AND Vulkan_shaderc_shared_DLL)
            ...
            set(_lus_shaderc_target Vulkan::shaderc_shared)
        endif()
    elseif(Vulkan_FOUND)
        ...
        elseif(Vulkan_shaderc_combined_LIBRARY)
            ...
            set(_lus_shaderc_target Vulkan::shaderc_combined)
        endif()
    endif()

    if(_lus_shaderc_target)
        ...
        message(STATUS "Vulkan rendering backend enabled (${_lus_shaderc_target})")
    else()
        set(LUS_ENABLE_VULKAN OFF CACHE INTERNAL "Vulkan backend available")
        if(Vulkan_FOUND)
            message(STATUS "Vulkan rendering backend disabled (shaderc not found)")
        else()
